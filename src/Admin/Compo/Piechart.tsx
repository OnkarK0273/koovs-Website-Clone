...
export default function Piechart({ mens, womens }: piechartProp) {
  ...
  useEffect(() => {
    ...
    //womens
    let sneakersarr = womens?.filter(
      (el: Product) => el.category === "sneakers"
    );
    let loungweararr = womens?.filter(
      (el: Product) => el.category === "loungwear"
    );
    let bodysuitsarr = womens?.filter(
      (el: Product) => el.category === "bodysuit"
    );
