...
export default function CustomerItems({data,setPage,page}:listingListProps) {
  ...
  const handleOpenDetails = (id:number, Image:string, Password:string, Name:string, Phone:string, Email:string ) => {
    setName(Name);
    setPass(Password);
    setImage(Image);
    msetId(id);
  ...
  const handleSubmit=()=>{
    ...
    dispatch(getAdminUserPatch(mid,image,pass,name,contact,Email)).then((res:any)=>{
      toast({
